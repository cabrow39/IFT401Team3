...
def generate_password_reset_email(user_email):
    ...
    return Message(subject='MarketMinds - Password Reset Requested',
...
def generate_confirmation_email(user_email):
    ...
    return Message(subject='MarketMinds - Confirm Your Email Address',
...
@users_blueprint.route('/trades')
@login_required
def user_trades():
    return render_template('users/trades.html')

@users_blueprint.route('/history')
@login_required
def user_history():
...
@users_blueprint.route('/portfolio')
@login_required
def user_portfolio():
